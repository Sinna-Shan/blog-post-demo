...
<template>
  <main class="w-screen h-screen bg-sky-500 flex items-center justify-center">
    <div class="bg-white rounded-md p-6 flex flex-col gap-y-2">
      <h1 class="text-3xl mb-6 text-center">Sign up to Writter</h1>
      <form @submit.prevent="handleSubmit" class="flex flex-col gap-2">
        ...
        <button class="bg-green-400 text-white h-10 rounded-md mt-5" type="submit">Sign up</button>
      </form>
      <p class="text-gray-800 text-sm mt-2">already have an account?</p>

      <RouterLink class="bg-blue-400 text-white h-10 rounded-md flex items-center justify-center" to="/login">Log in</RouterLink>
    </div>
  </main>
</template>
